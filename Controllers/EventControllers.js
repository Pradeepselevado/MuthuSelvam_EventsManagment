...
module.exports.createEvent = async (req, res) => {
    const { eventName, eventDate, location, description, endDate } = req.body;
    try {
        if (!eventName) {
            return res.json({ status: false, message: "eventName is required" });
        }
        const newEvent = new eventModal({ eventName, eventDate, location, description, endDate });
        await newEvent.save();
...
module.exports.updateEvent = async (req, res) => {
    const { EventId, eventName, eventDate, location, description, endDate } = req.body;
    try {
